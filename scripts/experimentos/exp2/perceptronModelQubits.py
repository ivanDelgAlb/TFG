...
def create_model(machine, X_train, X_test, y_train, y_test):
    ...
    directory = f'scripts/experimentos/exp2/model_qubits_{machine}.h5'
    model.save(directory)



def predict(machine, X_test, y_test):
    directory = 'scripts/experimentos/exp2/model_qubits_' + machine + '.h5'
...
for machine in machines:
    directory = 'scripts/experimentos/exp2/dataframe' + machine + '_experiment_2.csv'
    dataFrame = pd.read_csv(directory)

    filas_filtradas = dataFrame[(dataFrame['jensen-error'].notna())]

    X = filas_filtradas.drop(['n_qubits','depth','probability','t_gates','phase_gates','h_gates','cnot_gates','kullback_error','jensen-error','time'], axis=1)
